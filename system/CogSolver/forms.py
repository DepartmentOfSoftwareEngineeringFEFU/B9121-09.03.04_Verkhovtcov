...
class ApplicationForm(forms.ModelForm):
    ...
    class Meta:
        model = Application
        exclude = ['application_source', 'organizer_employee', 'event_schedule', 'installation_deinstallation']
        widgets = {
    ...
    def clean(self):
        ...
        if all_day:
            ...
            if not date_start or not date_end:
                raise forms.ValidationError("Для мероприятия на весь день необходимо указать даты начала и окончания")
            ...
            if time_start is None or time_end is None:
                raise forms.ValidationError("Необходимо указать время начала и окончания")
